<?php
if ( ! class_exists( 'Blogic_Author_Info_Widget' ) ) {
	/**
	 * Adds Blogic Author Info Widget.
	 */
	class Blogic_Author_Info_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$blogic_author_info_widget = array(
				'classname'   => 'widget adore-widget author-widget',
				'description' => __( 'Retrive Author Information Widget', 'blogic' ),
			);
			parent::__construct(
				'blogic_author_info_widget',
				__( 'Blogic Author Info Widget', 'blogic' ),
				$blogic_author_info_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'name' ) ); ?>"><?php esc_html_e( 'Author Name:', 'blogic' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description :', 'blogic' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'author_image_url' ) ); ?>"><?php esc_html_e( 'Author Image URL', 'blogic' ); ?></label>:<br />
				...
				<input type="button" class="select-img button button-primary" value="<?php esc_attr_e( 'Upload', 'blogic' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'blogic' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'blogic' ); ?>" style="margin-top:5px;" /><br/>
			</p>
			<p>
 ...
 * @since Blogic 1.0
 **/
function blogic_author_info_widget_image_upload_enqueue( $hook ) {
	...
	wp_enqueue_media();
	wp_enqueue_script( 'author-info-widget-image-upload-script', get_template_directory_uri() . '/assets/js/upload.js', array( 'jquery' ), BLOGIC_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'blogic_author_info_widget_image_upload_enqueue' );
