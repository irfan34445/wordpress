...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var Blogic_Image = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        blogic_file_frame = wp.media.frames.blogic_file_frame = wp.media({
          button: {
        ...
        blogic_file_frame.on( 'select', function() {

          var state = blogic_file_frame.state('author-info-insert-image');
